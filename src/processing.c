...
char *get_work_dir_path() {
    ...
    char *full_dir_path = malloc(strlen(dir_name) + strlen(home_dir) + 2);
    strcpy(full_dir_path, home_dir);
    strcat(full_dir_path, "/");
    strcat(full_dir_path, dir_name);
...
char *get_qs_dir_path() {
    ...
    char *full_dir_path = malloc(strlen(dir_name) + strlen(work_dir) + 2);
...
int make_dirs() {
    int code = check_dir(get_work_dir_path());

    if (code == EXIT_SUCCESS) {
        code = check_dir(get_qs_dir_path());
    }
...
int print_all_messages_from_queue(char *q_name) {
    ...
    while (fgets(message, 64, queue->file) != NULL) {
        printf("%s", message);
    }
